...
classes_bp = Blueprint("class_sessions", __name__)
...
def create_class_route():
    ...
    return jsonify(ClassResponseSchema(**dict(new_class)).dict()), 201


@classes_bp.get("/")
def get_all_classes_route():
    classes = get_all_classes()
    return jsonify([ClassResponseSchema(**dict(c)).dict() for c in classes]), 200


@classes_bp.get("/<int:class_id>")
def get_class_by_id_route(class_id):
    c = get_class_by_id(class_id)
    return jsonify(ClassResponseSchema(**dict(c)).dict()), 200
...
def update_class_route(class_id):
    ...
    return jsonify(ClassResponseSchema(**dict(updated)).dict()), 200
