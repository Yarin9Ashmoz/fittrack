    ...
    const fetchUserData = async () => {
      try {
        ...
        if (userResponse.data.role === "member" && canCreateEvaluation) {
          ...
          try {
            const trackingResponse = await personalTrackingAPI.getByMember(
              userId
            );
            ...
            <div>
              {user?.role === "member" ? (
                <>
                  {latestEvaluation ? (
                    ...
                    >
                      ...
                      <div style={{ marginBottom: "12px" }}>
                        <strong>Cleared for Training:</strong>{" "}
                        {latestEvaluation.cleared_for_training ? "Yes" : "No"}
                      </div>
                    ...
                    <div style={{ textAlign: "center", padding: "40px" }}>
                      <p style={{ color: "#64748b", marginBottom: "16px" }}>
                        No intake evaluation found
                      </p>
                      {canCreateEvaluation && (
                        <button
                          onClick={() =>
                            navigate(`/intake-evaluation/member/${userId}`)
                          }
                          style={{
                            padding: "10px 20px",
                            background:
                ...
                <p style={{ color: "#64748b" }}>
                  Intake evaluations are only for members
                </p>
              )}
            </div>
          )}
          ...
          {activeTab === "tracking" && (
            <div>
              {user?.role === "member" ? (
                        ...
                        >
                          ...
                          {entry.emotional_regulation && (
                            <div style={{ fontSize: "14px", color: "#64748b" }}>
                              Mood: {entry.emotional_regulation.mood} | Stress:{" "}
                              {entry.emotional_regulation.stress}
                            </div>
                          )}
                          {entry.physical_function && (
                            <div style={{ fontSize: "14px", color: "#64748b" }}>
                              Energy: {entry.physical_function.energy} | Sleep:{" "}
                              {entry.physical_function.sleep_hours}h
                            </div>
                          )}
                        </div>
                      ))}
                ...
                <p style={{ color: "#64748b" }}>
                  Personal tracking is only for members
                </p>
              )}
            </div>
          )}
